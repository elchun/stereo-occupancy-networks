...
from im2mesh.encoder.stereo_layers.cost_volume import CostVolume
from im2mesh.encoder.stereo_layers.hdc_functions import hdc_resnet_group
from im2mesh.encoder.stereo_layers.residual_blocks import PreactBasicResidualBlock, ResidualBlock, resnet_shortcut
from im2mesh.encoder.stereo_layers.residual_blocks import preact_resnet_group
from im2mesh.encoder.stereo_layers.matchability import Matchability
from im2mesh.encoder.stereo_layers.soft_argmin import SoftArgmin
from im2mesh.encoder.stereo_layers.transition_blocks import TransitionBlock
...
class TruncatedHdrnStereoEncoder(nn.Module):
    ...
    # def forward(self, left_image, right_image):
    def forward(self, x):
        left_image = x['l_image']
        right_image = x['r_image']
        # left_image = x[:, :3, :, :]
        # right_image = x[:, 3:, :, :]
